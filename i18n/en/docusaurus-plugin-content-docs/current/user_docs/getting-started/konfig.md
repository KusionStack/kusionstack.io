...
在安装完成 Kusion 工具之后，在 Konfig 根目录执行 `make check-all` 测试大库全部应用（参考 [Project & Stack ](/docs/user_docs/concepts/project-stack)），或者执行 `make check-http-echo` 测试 `appops/http-echo` 应用。
