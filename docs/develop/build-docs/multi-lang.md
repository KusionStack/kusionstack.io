...
1. 本地修改了源文件：执行 `make i18n-upload` 上传源文件变更
2. 登陆 crowdin 上的 KusionStack 翻译项目进行翻译：https://crowdin.com/project/kusionstack
3. 拉取 crowdin 上翻译文档文档更新：执行 `make i18n-download` 下载变更
4. 执行 `make i18n` 本地查看效果：http://127.0.0.1:8080

注意：第一次执行 `make i18n` 时可能会自动更新 Java 运行时环境（可能需要几十分钟）。
