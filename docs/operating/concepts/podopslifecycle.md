...
In PodOpsLifecycle, participants are classified into two roles: `operation controllers` and `resource controllers`.
- **Operation controllers** are responsible for operating Pods, such as Deployments and StatefulSets from Kubernetes, and CollaSets from Operating which intend to scaling, udating, or recreating Pods.
- **Resource controllers** handle resources around Pods, which may include traffic controllers. These controllers typically reconcile resources inside the Kubernetes cluster with external services, such as Kubernetes Service resources with the LB provider.
...
- **Completing**: After a Pod is created or updated and becomes ready, Operating marks its PodOpsLifecycle as the `Completing` phase. During this phase, the Pod is in a ready condition, prompting resource controllers to perform actions such as registering the Pod IP in the traffic route. Once all resource controllers complete their tasks, Operating sets the PodOpsLifecycle to the `ServiceAvailable` phase.
...
This section introduces how to develop operation controllers and resource controllers to interact with PodOpsLifecycle.
...
- The resource controller participates in PodOpsLifecycle before and after operating on a Pod, such as the Traffic controller, alert monitoring controller, and other controllers responsible for maintaining the Pod and application status. Users should develop a new resource controller only when there is a new type of service or status around the Pod that needs to be maintained, such as integrating with a new traffic provider.
...
There are two ways to develop a resource controller. 
