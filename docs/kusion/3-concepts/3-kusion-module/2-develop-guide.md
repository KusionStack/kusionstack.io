...
2. Identify module input parameters that should be configured by platform engineers in the [workspace](../4-workspace)
...
3. **Implement the gRPC generate interface.** The `generate` interface consumes the application developer's config described in the [`AppConfiguration`](../5-appconfiguration) and the platform engineer's config described in the [`workspace`](../4-workspace) to generate all infrastructure resources represented by this module.
...
More details can be found in the [`kusion mod push`](../../6-reference/1-commands/index) reference doc.
